...
SELECT
	...
	osm_id,
	fake,
	h.virtual_population population,
	round(transit_distance / 60 / 5) * 5 minutes5
...
WITH x AS (
        SELECT
         	h.id, h.centroid_913, h.virtual_population AS pop,
         	d.transit_distance,
...
select g.id, centroid_913, minutes, virtual_population
from 
	grouped g, houses h
where g.id=h.id;
